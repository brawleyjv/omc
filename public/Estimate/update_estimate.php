...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!$estimateId) {
            throw new Exception("Estimate ID is required");
        }
        ...
        // Update the estimate
        $result = $estimateModel->updateEstimate($estimateId, $estimateData, $materialsData, $customItems);

        if ($result) {
            // Redirect to estimate view page
            header("Location: " . BASE_URL . "Views/estimate/view_estimate.php?id=" . $estimateId . "&updated=1");
            exit();
        } else {
            throw new Exception("Failed to update estimate");
        }

    } catch (Exception $e) {
        error_log("Error updating estimate: " . $e->getMessage());
        $id = $_POST['estimate_id'] ?? '';
        header("Location: " . BASE_URL . "Views/estimate/edit_estimate.php?id=" . $id . "&error=" . urlencode($e->getMessage()));
        exit();
    }
} else {
